...
const mainLoop = async (event_metadata, UFC_scrape_tool) => {
    ...
    // Get data from ALL UFC events
    for(var d=0; d<ufc_event_url.length; d++){
        await get_event_fight_data(ufc_event_url[d], event_metadata, UFC_scrape_tool); // Get the last URL for the last completed UFC event
